...
public static class CaseConverter
{
    ...
    public static string ToPascalCase(string input) => Regex.Replace(input, "(\\B[A-Z])", " $1").Split().Select(x => char.ToUpper(x[0]) + x[1..].ToLower()).Aggregate("", (current, word) => current + word);
    ...
    public static string ToCamelCase(string input)
    {
        ...
        return words[0].ToLower() + string.Join(string.Empty, words.Skip(1).Select(w => char.ToUpper(w[0]) + w[1..]));
    }
